...
func updateEntrypoint(ctx context.Context, cmd *cli.Command) (err error) {
	...
	if !cmd.Bool("debug") && !cmd.Bool("debug-global") {
		if debug, derr := dbus.GetGlobalDebug(ctx); derr != nil {
			...
			if err = slog.SetupLogging(true); err != nil {
				return
			}
	...
	args := cmd.Args().Slice()
	if len(args) == 0 {
		zap.L().Debug("nothing to execute, exiting")
		return
	}
...
func processCredentialsDir(credsPath string, unitName string, managedCredentials map[string]string) (err error) {
	...
	for _, secretFileEntry := range existingSecrets {
		...
		us.Path, us.Replacement, err = createReplacementSecret(credsPath, credential, func(w io.Writer) (err error) {
			...
			if us.WrittenBytes, err = request.FetchCredential(credentialSocket, cr, w); err != nil {
